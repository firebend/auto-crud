...
public class ClientRequestTransactionManager : ISessionTransactionManager
{
    ...
    {
        _logger.LogDebug("{SessionId}: Completing {TransactionCount} transactions. {TransactionIds}", _sessionId,
        ...
        where TEntity : IEntity<TKey>
    {
        ...
        {
            _logger.LogDebug("{SessionId}: Transaction for {EntityName} - {TransactionId} is valid", _sessionId,
                typeof(TEntity).Name, transaction.Id);
            return transaction;
        }

        _logger.LogDebug("{SessionId}: Transaction for {EntityName} - {TransactionId} is invalid", _sessionId,
            typeof(TEntity).Name, transaction.Id);
    ...
    {
        _logger.LogDebug("{SessionId}: Getting or adding shared transaction for {EntityName} using key {Key}",
            _sessionId, typeof(TEntity).Name,
            key);
